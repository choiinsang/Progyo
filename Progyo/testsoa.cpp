...
void nsError         ( int error, char *domain );            /* report resolver errors */
void findNameServers ( char *domain, char *nsList[], int  *nsNum );    /* find a zone's name servers */
void addNameServers  ( char *nsList[], int  *nsNum, ns_msg handle, ns_sect section );     /* add name servers to our list */
void queryNameServers( char *domain,char *nsList[],int nsNum);   /* grab SOA records from servers */
void returnCodeError ( int rcode, char *nameserver );    /* report response message errors */
...
int main(int argc,char *argv[])
{
    ...
    int  nsNum = 1;        /* number of name servers in list */
...
int  *nsNum)
{
    ...
    if((responseLen =
           res_query(domain,      /* the zone we care about   */
                     ...
                     ns_t_mx,     /* Look up name server records*/
...
void
addNameServers(char *nsList[], int  *nsNum, ns_msg handle, ns_sect section)
{
...
void
queryNameServers(char *domain, char *nsList[], int nsNum)
{
    union {
        HEADER hdr;               /* defined in resolv.h */
        u_char buf[NS_PACKETSZ];  /* defined in arpa/nameser.h */
    ...
    for(nsNum-- ; nsNum >= 0; nsNum--){
        ...
        if(ns_msg_getflag(handle, ns_f_rcode) != ns_r_noerror){
            returnCodeError((int)ns_msg_getflag(handle, ns_f_rcode), nsList[nsNum]);
            continue; /* nsNum for-loop */
        }
        ...
        if (ns_rr_type(rr) != ns_t_mx) {
            (void) fprintf(stderr,
                ...
                nsList[nsNum], ns_t_mx, ns_rr_type(rr));
            continue; /* nsNum for-loop */
        }
         ...
         */
        ns_name_skip((const u_char**)&cp, ns_msg_end(handle));
        ns_name_skip((const u_char**)&cp, ns_msg_end(handle));
